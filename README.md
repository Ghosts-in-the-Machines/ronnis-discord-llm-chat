...
Generation settings such as `MAX_TOKENS`, `SEED`, `TOP_P`, `TOP_K`, penalties, `REASONING`, character cards, lorebooks, and memory hooks are configured in `.env`. Leave `REASONING` blank/off/none/omit to omit it, or set `REASONING=false` to send an explicit JSON false value for compatible providers. Boolean-like values such as `False`, `FALSE`, `no`, and `0` are normalized too. OpenRouter expects `reasoning` to be an object, so this client omits boolean values for OpenRouter and sends string values such as `medium` as `{"effort":"medium"}`. To request reasoning off through OpenRouter, use `REASONING={"effort":"none"}`; bare `REASONING=none` keeps this repo's normal meaning of omitting the parameter. For local OpenAI-compatible servers with template-specific options, set `OPENAI_EXTRA_BODY` to a JSON object such as `{"chat_template_kwargs":{"enable_thinking":false}}`. `STRIP_THINKING_BLOCKS=true` strips visible `<think>...</think>`, `<thinking>...</thinking>`, and similar wrappers from returned LLM text before Discord delivery. `ACK_MODE=auto` uses JSON decisions for high/advanced models and an `[ACK]` keyword for standard, low, or quantized models; the worker normalizes either form before deciding whether to send a Discord reply. Smaller, heavily quantized, or unusual local models may ignore reply/ack instructions or leak hidden reasoning; for Discord use, prefer a model and serving template that reliably follows chat instructions. `CTX_LIMIT` caps the recent Discord transcript characters in the prompt; character cards, lore, and retrieved memories are preserved ahead of that slice.
